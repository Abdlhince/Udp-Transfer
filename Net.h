...
// platform detection
// in order to get work socket for each platform.
...
#include <unistd.h>
...
#if PLATFORM == PLATFORM_WINDOWS
		WSADATA WsaData;
		return WSAStartup(MAKEWORD(2, 2), &WsaData) == NO_ERROR;   // fixed
	...
	//  Socket class provides a rich set of methods and properties for network communications.
	class Socket
	{
	public:
		Socket()
		...
		bool Open(unsigned short port)
		{
			...
			address.sin_port = htons((unsigned short)port);   //The htons function converts a u_short from host to TCP/IP network byte order
		...
		void Close()
		{
			if (socket != 0)
			{
				...
				close(socket);
#elif PLATFORM == PLATFORM_WINDOWS
				closesocket(socket);  // close eexisting socket
#endif
				socket = 0;
			}
		}
		// cehck socket is open or not
		bool IsOpen() const
		{
			return socket != 0;
		}
		// check address and port if both is correct 
		bool Send(const Address& destination, const void* data, int size)
		{
			...
			sockaddr_in address;   //  specifies a transport address and port for the AF_INET address family.
			address.sin_family = AF_INET;
			address.sin_addr.s_addr = htonl(destination.GetAddress());   //The htonl function converts a u_long from host to TCP / IP network byte order
			address.sin_port = htons((unsigned short)destination.GetPort()); //The htons function converts a u_short from host to TCP/IP network byte order
		...
		int Receive(Address& sender, void* data, int size)
		{
			...
			unsigned int address = ntohl(from.sin_addr.s_addr);  // The ntohl function converts a u_long from TCP/IP network order to host byte order
			unsigned short port = ntohs(from.sin_port);          // The ntohs function converts a u_short from TCP/IP network byte order to host byte orde
	...
	public:
		...
		virtual bool SendPacket(const unsigned char data[], int size)
		{
			...
			packet = (unsigned char*)malloc(size + 4); // dynamic memory allocation
			...
			ret = socket.Send(address, packet, size + 4);
			free(packet);
			return ret;
		}
		...
		virtual int ReceivePacket(unsigned char data[], int size)
		{
			assert(running);
			unsigned char* packet;
			packet = (unsigned char*)malloc((long long)size + 4);  // dynamic memory allocation
			Address sender;
			...
			if (sender == address)
			{
				...
				memcpy(data, &packet[4], (long long)bytes_read - 4);
				free(packet);
				return bytes_read - 4;
			}
			free(packet);
	...
	*/
	...
	public:
		...
		bool SendPacket(const unsigned char data[], int size)
		{
			...
			const int header = 12;
			unsigned char* packet;
			packet = (unsigned char*)malloc((long long)size + header);
		...
		int ReceivePacket(unsigned char data[], int size)
		{
			...
			if (size <= header)
				return false;
			unsigned char* packet;
			packet = (unsigned char*)malloc(size + header);
